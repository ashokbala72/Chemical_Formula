...
with tab12:
    ...
    if "mock_mode" not in st.session_state:
        st.session_state.mock_mode = True
    if "just_switched_to_live" not in st.session_state:
        st.session_state.just_switched_to_live = False
    ...
    if new_value != st.session_state.mock_mode:
        ...
        if not new_value:
            st.session_state.just_switched_to_live = True
        st.rerun()

    if not st.session_state.mock_mode and st.session_state.just_switched_to_live:
        st.success("🌐 Real-time data has been successfully loaded.")
        st.session_state.just_switched_to_live = False
